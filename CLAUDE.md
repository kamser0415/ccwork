...
## 개발 워크플로우 (이슈 단위 TDD 파이프라인)

**하나의 GitHub 이슈**를 구현할 때는 아래 7단계를 **순서대로** 따른다. 각 단계는 전용 skill/agent가 담당하며,
**단계마다 사람 승인 게이트가 있다.**

| #   | 단계                | 도구                         | 한 줄                                                            |
| --- | ------------------- | ---------------------------- | ---------------------------------------------------------------- |
| 1   | 시그니처 + 시나리오 | `/test-scenarios N` (skill)  | 계약 확정 → 정상/경계/예외 시나리오 도출                         |
| 2   | 실패 테스트 작성    | `/tdd-red N` (skill)         | 시나리오를 "실행되며 실패하는(RED)" 테스트로                     |
| 3   | 최소 구현           | `/tdd-green N` (skill)       | 빨간 테스트를 통과시키는 최소 코드, 전체 통과                    |
| 4   | AC 독립 검증        | `@ac-verifier N` (agent)     | "테스트 통과 ≠ AC 충족" — AC 의도 충족을 별도 판정               |
| 5   | 구조 개선           | `/tdd-refactor N` (skill)    | 동작 고정·구조만, 깨지면 즉시 롤백                               |
| 6   | 커밋 전 점검        | `/security-review N` (skill) | 타입·의존성 취약점·.env 노출 3분류                               |
| 7   | 통합                | git/gh                       | commit → PR `--base feature/<spec>` → squash merge → 이슈 클로즈 |

### 흐름 제어 규칙 (Claude가 지킬 것)

- **이 파이프라인을 이슈 작업의 기본 흐름으로 인식한다.** 이슈 번호로 개발을 시작하면 현재 어느 단계인지 파악하고,
  한 단계가 끝나면 **다음 단계를 제안**한다(무엇을·왜·어떤 명령으로).
- **자동으로 다음 단계로 넘어가지 않는다.** 각 단계는 사람 승인 게이트다 — 다음 skill/agent 호출은
  **사람이 명시적으로 지시하거나 `/명령`을 입력할 때만** 실행한다. 단계 내부 게이트(예: `test-scenarios`의
  시그니처·시나리오 승인, `tdd-refactor`의 대상 목록 승인)도 동일하게 존중한다.
- 단계를 건너뛰거나 순서를 바꾸자는 요청은 따르되 **그 사실과 위험을 먼저 짚는다**(예: RED 없이 GREEN은 false GREEN 위험).
- 4번(AC 검증)에서 갭이 나오면 보강 테스트를 임의로 넣지 말고 **2번(`tdd-red`)으로 되돌아갈지** 사람과 상의한다.

### 이슈 의존성 · 브랜치

- 이슈에 **선행 의존성**이 있으면(예: #3 depends on #2) **선행 이슈가 머지된 feature 브랜치에서 분기**한다(스택). 독립 이슈면 통합 기준 브랜치에서 분기.
- 통합은 `--base feature/<spec>`(스펙/베이스 브랜치)로 PR → **squash merge** → 이슈 클로즈.
- ⚠️ **GitHub 이슈 자동 클로즈는 "기본 브랜치(main) 머지 + `Closes #N` 키워드"일 때만** 작동한다.
  base가 `feature/<spec>`(비-기본 브랜치)면 자동 클로즈가 안 걸리므로 **`gh issue close N`으로 수동 종료**한다.
...
**모든 스타일/디자인 작업은 `design-system` 스킬을 사용하고 상세는 `docs/design/`를 참조한다**
(색·타이포·간격·반경·그림자·컴포넌트 스펙·Do/Don't·전환 매핑이 관심사별 파일로 분할돼 있다).
토큰의 런타임 정본은 `src/index.css`의 `@theme`. 별도 `tailwind.config.js` 없음(`@tailwindcss/vite`).

토큰 어휘(예): `bg-surface*`·`text-on-surface`·`bg-primary-container`(주 CTA)·`text-on-primary`·
`border-outline-variant`·`text-error`, 타이포 `text-headline-md`/`text-body-lg`/`text-label-md`,
반경 `rounded-card`/`rounded-field`, 간격 `gap-stack-xl`.

비협상 가드레일(상세·예외는 `docs/design/do-dont.md`):

- 색은 `@theme` 토큰만 — raw 색(`bg-gray-100`)·인라인 hex 금지.
- Tailwind 기본 반경 스케일(`--radius-xl/lg/md`) 덮어쓰기 금지. `--color-background` 변경 금지(필요 시 `surface`).
- 한글 텍스트에 Inter·Plus Jakarta Sans 금지(글리프 없음). 폰트는 Pretendard 유지, `--text-*`에 family 금지.
- 기존 별칭 토큰 7개(`bg-card` 등) 유지 — 삭제·일괄 치환 금지, 점진 전환만.

위 가드레일 중 **기계적 항목은 `scripts/check-design.mjs`로 결정적 강제**된다 — 편집 즉시(Claude
PostToolUse 훅, `.claude/settings.json`)와 커밋 시점(pre-commit `lint-staged`) 양쪽에서 차단한다.
단 "올바른 토큰 선택·미감·컴포넌트 패턴 일관성"은 코드로 판정 불가라 **모델 준수 + 사람 리뷰**로 남는다.
...
- `*.{ts,tsx}` → `eslint --fix` + `prettier --write` + **`node scripts/check-design.mjs`**(디자인 규칙 검사: raw 색·임의 색값·인라인 hex)
- `src/index.css` → **`node scripts/check-design.mjs`**(반경 스케일 오버라이드·금지 폰트·별칭 삭제·`@source` 검사)
- `*.{css,json,md}` → `prettier --write`만
- ESLint 자동 수정 불가 오류(미사용 변수 등)나 **디자인 규칙 위반**이 남으면 커밋이 **차단**된다(`scripts/check-design.mjs`가 exit 2).
